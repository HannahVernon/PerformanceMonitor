...
Only the visible tab loads (Lite's visible-only rule), and the 60-second timer refreshes it — with one exception: the **Recommendations** tab refreshes on tab activation, its Refresh button, and its own server-selector change only, never on the timer. Its findings change on the service's 30-minute analysis cadence, so a 60-second auto-refresh would be pointless churn (and would reset the incident expanders under the reader) — matching Lite's no-auto-timer behavior for that tab.
