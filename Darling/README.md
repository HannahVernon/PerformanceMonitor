...
| **Queries** | Three column-filterable sub-tabs over the last 24 hours — **Top Queries by Duration** (the full query-stats grid with in-grid bar cells for executions/CPU/duration/reads and a CPU-by-database breakdown), **Top Procedures by Duration**, and **Query Store by Duration** — each with a time-range slicer (drag to narrow the window), and a shared **Compare** control that overlays the current window against a baseline period (yesterday, last week, or same day last week), flagging new and vanished queries. More sub-tabs (performance trends, active queries, query heatmap) are coming |
...
The three aggregate tabs — **Overview** and **Alerts** span every server; **Recommendations** is scoped to the sidebar's single-click selection:
...
Only the visible tab loads (Lite's visible-only rule). The Recommendations and Alerts tabs and the visible server tab's active inner tab refresh every 60 seconds; the Overview refreshes on its own faster 30-second timer (Lite's Overview cadence).
