...
public static class FactAdvice
{
    ...
    private static Dictionary<string, AdviceBlock> BuildAdviceTable()
    {
        ...
        t["SOS_SCHEDULER_YIELD"] = new AdviceBlock(
            ...
            Investigation:
                "SOS_SCHEDULER_YIELD is logged when a worker voluntarily yields after using its full 4 ms scheduling quantum (cooperative scheduling), so a high count means workers are running CPU-bound for long stretches — most often large in-memory scans chewing through pages that are already in the buffer pool. It is NOT by itself proof the box is short of CPU: true CPU oversubscription shows up as signal-wait time and a deep runnable queue, not as SOS_SCHEDULER_YIELD. Open the CPU tab to see the SQL vs. other-process split over the analysis window. The drill-down `top_cpu_queries` is already attached: those five queries account for the CPU being burned. Call `get_cpu_utilization` for the per-minute trend and `get_top_queries_by_cpu` for the cached-plan view of the offenders.",
            Remediation:
                "Tune the queries in `top_cpu_queries` before sizing more hardware — the wait points at queries burning CPU (typically scans that should be seeks), and cutting that work is almost always cheaper than adding cores. Fix the missing indexes, scans, and parameter sniffing the plan analysis surfaces. If CXPACKET is co-elevated, trivial queries are likely going parallel: raise `cost threshold for parallelism` to 50 and cap `MAXDOP` at the logical processors in a single NUMA node (≤ 8).");
