...
public static class FactAdvice
{
    ...
    private static Dictionary<string, AdviceBlock> BuildAdviceTable()
    {
        ...
        t["CXPACKET"] = new AdviceBlock(
            ...
            Investigation:
                "The collector groups every CX* wait (CXPACKET, CXCONSUMER, CXSYNC_PORT, CXSYNC_CONSUMER) into one CXPACKET fact (`Lite/Analysis/DuckDbFactCollector.cs:GroupParallelismWaits`). That grouping hides producer/consumer skew — CXCONSUMER on its own specifically means one branch is doing the work while siblings stall waiting for rows. If the per-execution duration of the offending queries is closer to their CPU time than CPU÷DOP would predict, parallelism is mostly contending with itself, not paying for itself. Open the Wait Stats tab to see the breakdown over the analysis window, and call `audit_config` to check CTFP and MAXDOP. The QUERY_HIGH_DOP amplifier flags queries running at DOP > 8 — `get_top_queries_by_cpu` with `parallel_only=true` ranks them.",
            Remediation:
        ...
        t["LCK"] = new AdviceBlock(
            ...
            Investigation:
                "LCK groups X (exclusive), U (update), IX (intent exclusive), SIX, and BU waits — the writer/writer side of lock contention. Unlike LCK_M_S/LCK_M_IS (readers blocked by writers, fixable with RCSI), these are real serialization points: two writers genuinely cannot proceed at the same time. The drill-down `lock_mode_breakdown` is already attached: every LCK% wait type ranked by `total_wait_ms` and `waiting_tasks` over the analysis window — that tells you exactly which lock modes are dominating. Open the Wait Stats tab to see the LCK family on the chart. `get_waiting_tasks` returns the live waiting list with lock details.",
            Remediation:
