...
/// significance filter); counts floor at 0 and the time axis is bound to the toolbar's settable window.
/// Charts ride the shared ChartStyle / ChartPalette and the same ApplyTheme / ChartHoverHelper / SeriesColors
/// helpers as the Latch-Spinlock and Blocking trend charts, so the look matches Lite's other charts exactly.
/// </summary>
public partial class ServerTab : UserControl
{
        ...
        Func<SystemHealthRecord, long?> selector, string label, string colorHex, double xMin, double xMax)
    {
        ...
        {
            var xs = data.Select(d => d.EventTime!.Value.AddMinutes(UtcOffsetMinutes).ToOADate()).ToArray();
    ...
    /// plotting the spinlock backoffs over event time, with a bottom legend — mirrors the Dashboard's grouped
    /// multi-series chart (backoffs, or 1 when the count is absent).
    /// </summary>
    private void RenderSickSpinlocksChart(List<SystemHealthRecord> data, double xMin, double xMax)
    {
        ...
        else
        {
            ...
            foreach (var spinlockType in spinlockTypes)
            {
                ...
                var xs = typeData.Select(d => d.EventTime!.Value.AddMinutes(UtcOffsetMinutes).ToOADate()).ToArray();
    ...
    /// the Dashboard's CPU-comparison chart (System CPU on the cycling accent, SQL CPU on the shared SqlCpu
    /// identity color).
    /// </summary>
    ...
    /// (placeholder accent color), matching the Dashboard's no-data placeholder text.</summary>
