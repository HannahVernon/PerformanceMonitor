        ...
        using (var duckConnection = _duckDb.CreateConnection())
        {
            ...
            using (var appender = duckConnection.CreateAppender("wait_stats"))
            {
                foreach (var stat in waitStats)
                {
                    ...
                    var deltaWaitingTasks = _deltaCalculator.CalculateDelta(serverId, "wait_stats_tasks", deltaKey, stat.WaitingTasks, collectionTime: collectionTime, maxGapSeconds: 300);
                    var deltaWaitTimeMs = _deltaCalculator.CalculateDelta(serverId, "wait_stats_time", deltaKey, stat.WaitTimeMs, collectionTime: collectionTime, maxGapSeconds: 300);
                    var deltaSignalWaitTimeMs = _deltaCalculator.CalculateDelta(serverId, "wait_stats_signal", deltaKey, stat.SignalWaitTimeMs, collectionTime: collectionTime, maxGapSeconds: 300);
