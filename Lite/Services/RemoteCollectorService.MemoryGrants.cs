        ...
        using (var duckConnection = _duckDb.CreateConnection())
        {
            ...
            using (var appender = duckConnection.CreateAppender("memory_grant_stats"))
            {
                foreach (var r in rows)
                {
                    ...
                    var deltaTimeouts = _deltaCalculator.CalculateDelta(serverId, "memory_grants_timeouts", deltaKey, r.TimeoutErrorCount, collectionTime: collectionTime, maxGapSeconds: 300);
                    var deltaForced = _deltaCalculator.CalculateDelta(serverId, "memory_grants_forced", deltaKey, r.ForcedGrantCount, collectionTime: collectionTime, maxGapSeconds: 300);
