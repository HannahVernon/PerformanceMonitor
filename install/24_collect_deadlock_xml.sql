...
Deadlock XML collector
Collects raw deadlock XML from ring_buffer target for later analysis with sp_BlitzLock
...
IF OBJECT_ID(N'collect.deadlock_xml_collector', N'P') IS NULL
BEGIN
    EXECUTE(N'CREATE PROCEDURE collect.deadlock_xml_collector AS RETURN 138;');
END;
GO

ALTER PROCEDURE
    collect.deadlock_xml_collector
(
    @minutes_back integer = 15, /*How many minutes back to collect deadlocks*/
...
BEGIN
    ...
    BEGIN TRY
        ...
        */
        ...
        BEGIN
            ...
            (
                ...
                0,
                0,
                N'Table collect.deadlock_xml does not exist, calling ensure procedure'
            );
            ...
            IF OBJECT_ID(N'collect.deadlock_xml', N'U') IS NULL
            BEGIN
                RAISERROR(N'Table collect.deadlock_xml still missing after ensure procedure', 16, 1);
                RETURN;
            END;
        END;
        ...
        */
        ...
        IF @debug = 1
        BEGIN
            RAISERROR(N'Collected %I64d deadlock XML events', 0, 1, @rows_collected) WITH NOWAIT;
        END;
    ...
    BEGIN CATCH
        ...
        RAISERROR(N'Error in deadlock XML collector: %s', 16, 1, @error_message);
    END CATCH;
END;
GO

PRINT 'Deadlock XML collector created successfully';
PRINT 'Reads from ring_buffer target of PerformanceMonitor_Deadlock session';
PRINT 'Stores raw deadlock XML for later analysis with sp_BlitzLock';
GO
