...
- **Darling: the analysis-store foundation — the analysis engine's Postgres storage lands ahead of the pipeline port** ([#1262]) — Phase-5 analysis slice AN1. A V4 "analysis-tables" migration creates `analysis_findings` in Lite's schema-v3 shape PLUS the Dashboard's richer `remediation_action_json` column (the BUILT RemediationAction persisted on the row — recommendations rebuild D2), with no primary key (the collector tables' hypertable/COPY reasoning) and Lite's index columns carried over; the PK'd `analysis_muted` registry (small registries keep their keys, like V2's servers); and passthrough `v_<table>` views for every collected table Lite's analysis reads — in Lite those views union the hot DuckDB table with the parquet archive, Darling has no parquet tier, so plain passthroughs let the ported analysis SQL run VERBATIM in the next slices. Seventeen views, not the fourteen the fact collectors read: Lite's drill-down/storage collectors also read `v_tempdb_stats`, `v_query_snapshots`, and `v_database_config`, and each view target is pinned against the shared collector catalog so a passthrough can never reference a nonexistent table. The new `PerformanceMonitor.Darling.Analysis` library holds `PgFindingStore` — the Dashboard twin's method surface and semantics (the two-phase FilterMuted → Insert write, so remediation actions are built between the phases and persisted through the SAME shared `AlertContextSerializer` both apps use, byte-identical round-trip) plus Lite's single-pass `SaveFindingsAsync` composed from the two phases, finding/mute ids from the shared `CollectionIdGenerator`, every timestamp naive-UTC Kind-Unspecified with "now" always a bound parameter (never a bare `now()`), and two deliberate richer-twin reconciliations (findings keep the story's `DatabaseName`, which the Dashboard twin drops; `GetLatestFindingsAsync` returns the remediation column the Dashboard twin omits) — and `PgPlanFetcher`, the Dashboard's live plan-cache fetch (same query, timeouts, and null-degrade) behind a `serverId → connection string` resolver seam the worker will supply from its per-server runtimes, so the analysis library never references the service. Pinned ungated (migration shape, PG dialect, fetcher seam/degrade) and exercised end-to-end against a dev Postgres gated on DARLING_TEST_PG: migrate → insert with a built action → read back intact → mute excludes → unmute restores → retention purge → a `v_wait_stats` passthrough SELECT. The analysis pipeline itself (fact collectors, detectors, orchestration) is the next slice . The AN2b slice lands the anomaly half of that pipeline: `PgAnomalyDetector` (Lite's nine detectors plus the baseline-data gate — same fact keys, metadata, thresholds, and per-detector error tolerance) and `PgBaselineProvider` (the 30-day hour×day-of-week baseline model with tier collapse, hysteresis, caching, and all eleven metric queries) run Lite's SQL against the V4 views with every time bound a naive-UTC parameter — the arc's only genuine dialect work being the four DuckDB `QUALIFY` exclusions rewritten as window-in-CTE + outer-WHERE (Postgres has no QUALIFY; each site documents the DuckDB original and preserves its row selection exactly, LAG computed over the pre-exclusion rowset so only the first zero after a busy sample drops and genuine idle zeros survive), pinned ungated (method surfaces vs Lite, a no-QUALIFY/no-bare-now dialect sweep, per-site rewrite structure) and proven against a live Postgres gated on DARLING_TEST_PG — a planted hour×dow wait history shows the counter-reset (restart) sample excluded from both rewritten wait baselines by sample count AND mean while a genuine idle zero survives, and a planted anomalous window fires the ported wait detector end-to-end. Slice AN2a ports the fact collector itself: `PgFactCollector` is Lite's `DuckDbFactCollector` method-for-method (all 28 collect methods across the same seven partial files — same fact keys, values, metadata shapes, emission order, and swallow-everything error posture, including the shared blocking-chain reconstruction with its DMV-snapshot fallback and the WS4 plan-XML advisories through the shared `PlanAdvisoryAggregator`) with every query byte-identical against the passthrough views, the only deviations documented in-code (DuckDB's `AcquireReadLock` dropped — Postgres MVCC needs none; parameters bound positionally with naive-UTC Kind-Unspecified timestamps; DuckDB's BigInteger-tolerant reads simplified — Npgsql returns `decimal`; `any_value()` kept, in Postgres since 16 with product minimum 17), pinned by `PgFactCollectorTests` ungated (the literal Lite method-surface inventory so drift fails loudly, SQL hygiene — no QUALIFY / bare NOW()/CURRENT_TIMESTAMP / read_parquet / UNION ALL BY NAME, $N positional only — and every FROM/JOIN target resolving to a V4 view or catalog table) plus a gated end-to-end that plants wait-stats and CPU rows, runs the full `CollectFactsAsync`, asserts the expected fact keys/values in Lite's emission order, and proves an empty store yields zero facts with no exception. The final slice AN3 lands **the pipeline itself — the headless service now analyzes on schedule and notifies findings through the shared path**: `DarlingAnalysisService` orchestrates Lite's pipeline order (24h-data-span gate → facts → anomalies → scoring → story traversal → composed StoryText → incident clustering) with the Dashboard twin's richer save phase adopted (mute-filter WITHOUT inserting → drill-down enrich the survivors → build + attach each finding's `RemediationAction` → insert with `remediation_action_json` persisted) on host-UTC windows (Darling's collectors stamp host UTC exactly like Lite's — no server-clock probe); `PgDrillDownCollector` is Lite's `DrillDownCollector` ported method-for-method (all 17 collectors across the same five partial files — deadlocks/blocking/reconstructed chains through the shared pair-row fragments, config issues, live plan analysis via `PgPlanFetcher` + the WS4 plan advisories, spike/CPU/spill/parameter-sensitivity/plan-regression/bad-actor/pending-grant queries, IO/autogrowth/tempdb breakdowns) with every query byte-identical against the passthrough views and the same documented PG deviations as AN2a; the worker runs a per-server analysis due-loop on Lite's defaults hardcoded (30-minute interval, 120-second timeout, 4-hour window; an in-flight guard so a hung pass is never relaunched; first run immediately after connect — the pipeline's own data-span gate no-ops a fresh store until it has 24h of history) and routes findings through the shared `AnalysisNotificationService` → the new `DarlingFindingAlertSender` (Lite's `EmailAlertService` finding cadence over the same `EmailSendCore`/webhook flow and PG alert history the alert engine uses: severity ≥ 1.5 notifies, 360-minute per-finding cooldown seeded across restarts, one combined history row per alert regardless of channel; delivery gated by the master `alerts.enabled` while analysis itself always runs + persists); the worker's daily retention sweep now also invokes the finding store's 30-day cleanup (both apps declare it but never schedule it — a 24/7 service must, or `analysis_findings` grows unbounded). Pinned ungated (the twins' orchestrator surface with Lite's 24h gate default and multi-server data-span SQL, the literal Lite drill-down method inventory, the AN2a SQL-hygiene sweep over all 18 drill-down queries with every FROM/JOIN target resolving to a V4 view or collector table, the worker cadence and notify defaults against Lite's `App` values) and proven end-to-end against a live Postgres gated on DARLING_TEST_PG: plant a week-spanning wait history with an anomalous recent window, run the full `AnalyzeAsync`, and assert findings persist to `analysis_findings` with severity/story/remediation columns round-tripping, a recording sender receives exactly the above-threshold findings (and the cooldown suppresses an immediate repeat), a second run respects `MuteFindingAsync`, and an empty server no-ops on the insufficient-data gate. Slice AN4 puts the analysis surface on the wire: **the service now exposes the six analysis MCP tools** — analyze_server, get_analysis_facts, compare_analysis, audit_config, get_analysis_findings, mute_analysis_finding — the SAME tool surface Lite and the Dashboard expose, with each tool body mirroring Lite's field-for-field (same response envelopes, the #1224 miss-vocabulary Status envelope, the shared FactAdvice/CoFiredSummary composition and the per-app ToolRecommendations copy) so MCP clients see one consistent product across all three SKUs; hosting mirrors Lite's model exactly (ModelContextProtocol.AspNetCore 1.4.0, Streamable HTTP on localhost, stateless mode + Gemini-compatible schemas for #1074, the port-in-use pre-check) as an optional hosted service gated by a new darling.json `mcp` section — **default OFF** (`{ "mcp": { "enabled": false, "port": 5152 } }`; a headless service opens no local port unprompted, and 5152 completes the family's non-colliding port map: Dashboard 5150, Lite 5151); server names resolve through the Postgres `servers` registry the worker upserts on connect (Lite's ServerResolver semantics — sole-server auto-select, exact-beats-partial on storage or display name, the available-servers error listing with the `[Read-Only]` tag derived from the `:RO` storage-name suffix — with the registry's shared-hash server_id passed through, never re-derived), and analyze_server's live plan fetches resolve connection strings from darling.json lazily per fetch. Pinned ungated (the six-tool surface via the MCP attributes — all static, the Gemini-registration requirement; the config default-off + family port; the full resolver semantics over fake registry rows) and end-to-end against a live Postgres gated on DARLING_TEST_PG: register servers the way the worker does, plant a persisted finding, call the tool methods directly — the finding round-trips through Lite's envelope field-for-field, the empty server returns the shared "empty" Status envelope, partial/display/unknown names resolve per the registry semantics, mute_analysis_finding writes the mute row and returns the muted envelope, and the mute then filters the same story from a subsequent analysis-run save
