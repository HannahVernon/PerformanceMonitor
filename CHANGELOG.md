...
- **Darling viewer: the CPU series now aligns with every other timeline on non-UTC servers** ([#1262]) — `cpu_utilization_stats.sample_time` is the monitored server's LOCAL wall clock (`SYSDATETIME()` on the server, minus each ring-buffer sample's age), but the viewer's CPU read handed it to `ToLocalTime`, which assumes naive-UTC input — so both consumers, the CPU inner tab and the Overview's correlated CPU lane, plotted the whole CPU series shifted by the server's UTC offset relative to every `collection_time`-based lane (four hours off on a US Eastern server). `GetCpuUtilizationAsync` now de-skews `sample_time` to naive UTC inside the read: within a collection batch (keyed on the shared `collection_time` — `collection_id` is a per-row column in the Darling store, not a batch id) the newest sample is at most ~1 minute older than the collection instant, so `round(MAX(sample_time) over the batch - collection_time)` to the nearest 15 minutes recovers the server's UTC offset (covering half/quarter-hour zones and absorbing the sub-minute anchor age), and subtracting it turns each local `sample_time` into true naive UTC. A UTC server yields a zero offset, byte-identical to the prior read. Pinned by an ungated SQL-shape test (window MAX + 15-minute rounding) and a gated live-Postgres round-trip that plants a simulated -4h batch and a UTC batch and asserts both land back on their `collection_time`
